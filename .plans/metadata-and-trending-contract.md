...
*   **Next Release Dates:** AniList provides a `nextAiringEpisode` object containing a precise Unix timestamp (e.g., `1719000000`). The UI converts this to a live countdown: *"Episode 5 airs in 2 days, 4 hours."*
...
*   **Release Dates:** TMDB provides `release_date` (theatrical) and `status` ("Returning Series" vs "Ended").
...
*   **Anime (Absolute Mapping):** Anime rarely uses "Season 2, Episode 1". They use absolute numbering (e.g., *One Piece Episode 1159*). The `AniList ID` covers the entire series.
*   **Western Series (SxE Mapping):** Western shows use strict Season and Episode numbering (e.g., *Breaking Bad S04E05*). TMDB natively supports this. When passing this to Vidking or Rivestream, we pass both the `TMDB_ID`, `season: 4`, and `episode: 5`.
