...
When `yt-dlp` is used *just* to extract a stream URL (which is what `mpv` does behind the scenes), it acts purely as a web scraper. It does **not** download, encode, or transcode the video. It simply:
...
- *Crucial:* Do not close the browser after search. Keep the `page` object alive in a class property so that the `getEpisodes` and `getSources` methods can reuse the established session, maintaining the speed of the scratchpad.
