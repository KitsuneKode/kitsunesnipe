...
*   **The JIT Trigger (Just-In-Time):** If a user selects an episode from Anikai or Miruro, Kunai launches a hidden Playwright instance *at that exact second*, bypasses the Cloudflare challenge, extracts the stream URL, passes it to `mpv`, and **instantly kills the Playwright process.**
...
3. **The Premium Cloud Extractor ($3/mo SaaS):** For mobile web users who *really* want an obscure anime that only exists on `mp4upload`, we offer a premium subscription. This unlocks a backend microservice we host (`extract.kunai.app`) that runs `yt-dlp` on our servers, rips the link, and sends it to their phone.
...
```
*Tests are co-located* with their scrapers (`miruro.ts` next to `miruro.test.ts`).