...
## GraphQL Schema — Show Type Metadata

The `Show` type in the allanime GraphQL API carries rich metadata (discovered via `__type` introspection on `api.allanime.day/api`). The `shows` search query and `show` detail query both return this type.
...
### Fields That Return Null

- `season{year season}` — nested field selection fails on Show type
- `episodeInfos(showId)` — GraphQL query type fields exist but return null (may require auth or different schema version)
- `episodes(showId translationType episodeNumStart episodeNumEnd)` — same as above
