...
export const CinebyAnime: ApiProvider = {
  ...
  async search(query): Promise<ApiSearchResult[]> {
    ...
    const raw  = (data as any)?.data?.animes ?? (data as any)?.results ?? (data as any)?.animes ?? [];

    return (raw as any[]).map((a): ApiSearchResult => ({
  ...
  async resolveStream(id, _type, _season, episode, opts: ResolveOpts) {
    // cineby.sc/anime/{hianime-slug}?episode={n}&play=true
    // The `play=true` param wakes the player. A simulated click is still
    // needed (same as regular Cineby) — embedScraper handles that via the
    // existing scrapeStream → needsClick path on the EMBED_PROVIDER.
    const url = `https://www.cineby.sc/anime/${id}?episode=${episode}&play=true`;
    return opts.embedScraper(url);
  },
};
